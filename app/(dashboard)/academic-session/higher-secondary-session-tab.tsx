...
export function HigherSecondarySessionTab() {
  ...
  const { data: audit, isLoading: isLoadingAudit, refetch } = useQuery({
    queryKey: ["session-readiness-hs", fromYear, examName, minPassPercentage],
    queryFn: () => getSessionReadiness(fromYear, examName, minPassPercentage, "higher_secondary"),
  });

  const transitionMutation = useMutation({
    mutationFn: (params: any) => executeSessionTransition({ ...params, section: "higher_secondary" }),
    onSuccess: (data) => {
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      {transitionResult && (
        <div className="rounded-2xl border border-emerald-300 bg-emerald-50 dark:bg-emerald-950/40 p-5 space-y-3 shadow-sm">
          <div className="flex items-center gap-2.5 text-emerald-800 dark:text-emerald-300 font-semibold text-sm">
            ...
            Academic Session {toYear} Successfully Initiated for Classes XI–XII!
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-1 text-center">
            <div className="bg-white/80 dark:bg-black/20 rounded-xl p-2.5 border border-emerald-200">
              <p className="text-[11px] text-muted-foreground">Promoted (XI)</p>
            ...
            <div className="bg-white/80 dark:bg-black/20 rounded-xl p-2.5 border border-amber-200">
              <p className="text-[11px] text-muted-foreground">Sent Up H.S. (Cl XII)</p>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs bg-muted/40 border border-border/80 rounded-2xl p-4 shadow-2xs">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="font-bold text-foreground">Higher Secondary Section (Classes XI–XII) Promotion Policy</span>
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              <Badge variant="outline" className="text-[10px] bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300 border-emerald-200">Class XI ➜ XII: Pass Required (≥ {minPassPercentage}%)</Badge>
              ...
              <Badge variant="outline" className="text-[10px] bg-amber-50 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300 border-amber-200 font-medium">
                Class XII ➜ Sent Up H.S. (C.C.H.S.): Pass Required (≥ {minPassPercentage}%)
              </Badge>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="rounded-2xl border bg-card p-4 space-y-2 shadow-2xs">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Pass Cutoff (Class XI & XII)</p>
          <div className="flex items-center gap-2">
            <input
              type="number"
              min={0}
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            <div className="flex items-center gap-2 text-rose-600">
              ...
              <DialogTitle>Confirm Higher Secondary Section (Classes XI–XII) Transition</DialogTitle>
            </div>
